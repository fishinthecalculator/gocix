...
(define (tests-for-current-gocix-guix source commit)
  "Return a list of tests for perform, using gocix built from SOURCE, a channel
instance."
...
the 'TESTS' environment variable."
  ...
  (let* ((source (local-file source
                             ...
                             (or (git-predicate source)
                                 (const #t))))
         (tests  (tests-for-current-gocix-guix source commit)))
